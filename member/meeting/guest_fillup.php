...
include('guest_server.php');
	if (isset($_GET['edit'])) {
		$id = $_GET['edit'];
		$update = true;
		$record = mysqli_query($db, "SELECT * FROM guest WHERE id=$id");

		if (@count($record) == 1 ) {
			$n = mysqli_fetch_array($record);
			$first_name = $n['first_name'];
      $last_name = $n['last_name'];
      $food_choice = $n['food_choice'];
		}
	}
...
<?php if (isset($_POST['no'])) {
    ...
    $email = $_SESSION['email'];
    mysqli_query($db, "INSERT INTO member_meeting(email,food_choice,presence,meeting_code) VALUES ('$email','$food_choice','$presence','$meeting_code')");
...
<div class="container">
<form method="post" action="guest_server.php" >
    <div class="form-group">
    <input type="hidden" name="id" value="<?php echo $id; ?>">
      <label for="first_name">First Name:</label>
      <input type="text" class="form-control w-25" name="first_name" value="<?php echo $first_name; ?>">
    </div>
    <div class="form-group">
      <label for="last_name">Last Name:</label>
      <input type="hidden" name="email" value="<?php echo $email; ?>">
      <input type="text" class="form-control w-25" name="last_name" value="<?php echo $last_name; ?>">
    </div>
     <div class="form-group">
      <div class="radio">
        <label for="last_name">Food Choice:</label></br>
       <label><input type="radio" name="food_choice" value="Veg" checked>Veg</label>
      </div>
      <div class="radio">
     <label><input type="radio" name="food_choice" value="Non Veg">Non-Veg</label>
    </div>
    <div class="radio">
    <label><input type="radio"name="food_choice" value="Fish">Fish</label>
    </div>
    </div>
     <div class="form-group">
     <?php if ($update == true): ?>
			<br><button  class="btn btn-dark" type="submit" name="update">update</button>
		<?php else: ?>
       <button type="submit" class="btn btn-dark" name="add_guest" >Add Guest</button>
      </div>
      <div class="form-group">
...
<?php
$email = $_SESSION['email'];
$results = mysqli_query($db, "SELECT * FROM guest WHERE email='$email'"); 
?>
    ...
    <td>
      <a href="guest_fillup.php?edit=<?php echo $row['id']; ?>" class="edit_btn" >Edit</a>
    </td>
    <td>
      <a href="guest_server.php?del=<?php echo $row['id']; ?>" class="del_btn">Delete</a>
    </td>
      </tr>
      <?php } ?>      
    </tbody>
