...
<body>
  ...
  $results = mysqli_query($db, "SELECT * FROM meeting");
  ?>
    <div class="container pt-4">
    <form method="post" action="report_server.php">
      <div class="dropdown pb-4 form-group">
        <label for="remainder_meeting">Remainder Meeting</label>
        <button class="btn btn-dark" type="button" data-toggle="dropdown">
          <select name="meeting_date"><?php while ($row = mysqli_fetch_array($results)) { ?>
              <option><?php echo $row['meeting_date'] ?></option>
            <?php } ?>
          </select>
      </div>
      <div class="radio">
        <label><input type="radio" name="choice" checked> Attandance</label>
      </div>
      <div class="radio">
        <label><input type="radio" name="choice">Food Choice</label>
      </div>
      <div class="radio disabled">
        <label><input type="radio" name="choice">Guest Invoice</label>
      </div>
      <div class="radio disabled">
        <label><input type="radio" name="choice">Membership Invoice</label>
      </div>
      <div class="form-group pb-5 pt-4">
        <button type="submit" class="btn btn-warning" name="submit">Send Email</button>
      </div>
      </form>
    </div>
